...
class Options_Machine {
	...
	function __construct($options) {
		...
		$this->Menu = $this->smof_menu($options);
		$this->Defaults = $this->smof_defaults($options);
	 ...
	 * @since 1.4.1
	 */
	public static function smof_menu($options) {
		$menu = '';
	 ...
	 * @since 1.4.1
	 */
	public static function smof_defaults($options) {
		$defaults = '';
		foreach ($options as $value) {	
	 ...
	 * @return array
	 */
	public static function smof_machine($options) {
		...
		foreach ($options as $value) {
			...
			switch ( $value['type'] ) {
				...
				case 'text':
					...
					$t_value = isset($data[$value['id']]) ? stripslashes($data[$value['id']]) : '';
					
					$mini = isset($value['mini']) ? 'mini' : '';
					
					$output .= '<input type="text" class="of-input '.$mini.'" name="'.$value['id'].'" id="'. $value['id'] .'" type="'. $value['type'] .'" value="'. htmlspecialchars($t_value) .'" />';
				break;
				
				//select option
				case 'select':
					$mini = isset($value['mini']) ? 'mini' : '';
					$multi = isset($value['multi']) ? 'multiple="multiple"' : '';
					$selected = isset($data[$value['id']]) ? $data[$value['id']] : $value['std'];
					
					
					$output .= '<div class="select_wrapper ' . $mini . '">';
					$output .= '<select '. $multi .' class="select of-input" name="'.$value['id'].'" id="'. $value['id'] .'">';
					foreach ($value['options'] as $select_ID => $option) {			
						$output .= '<option id="' . $select_ID . '" value="'.$select_ID.'" ' . selected($selected, $select_ID, false) . ' />'.$option.'</option>';
					 } 
					$output .= '</select></div>';
				break;
				...
				case 'multiselect':
					$mini = isset($value['mini']) ? 'mini' : '';
					$selected_data = $data[$value['id']] ? $data[$value['id']] : array();
					
					$output .= '<div class="multiselect_wrapper ' . $mini . '">';
					$output .= '<select multiple="multiple" class="select of-input" name="'.$value['id'].'[]" id="'. $value['id'] .'">';
					foreach ($value['options'] as $select_ID => $option) {
						$selected = '';
						if(in_array($select_ID, $selected_data)) $selected = 'selected="selected"';			
				...
				case 'textarea':	
					...
					if(isset($value['options'])){
						...
						$ta_value = stripslashes( isset($data[$value['id']]) ? $data[$value['id']] : $value['std']);			
				...
				//colorpicker option
				case 'color':
					$color = isset($data[$value['id']]) ? $data[$value['id']] : $value['std'];
				...
				case 'slider':
					...
					$slides = isset($data[$value['id']]) ? $data[$value['id']] : array();
					$count = count($slides);
				...
				case 'sorter':
					...
					if ($sortlists) {
						...
						foreach ($sortlists as $group=>$sortlist) {
							...
							$output .= '<h3 class="'.$group.'">'.$group.'</h3>';
				...
				case 'backup':
					...
					$output .= '<p><strong>'. __('Last Backup : ', 'aq10e').'<span class="backup-log">'.$log.'</span></strong></p></div>'."\n";
	...
	public static function optionsframework_slider_function($id,$std,$oldorder,$order,$int){
		...
		$slider = '';
		$slide = array();
	    $slide = isset($data[$id]) ? $data[$id] : '';
		
	    if (isset($slide[$oldorder])) { $val = $slide[$oldorder]; } else {$val = $std;}
		
		//initialize all vars
		$slidevars = array('title', 'bg', 'url','mode','description');
		...
		$slider .= '<div class="slider_bg">';
			$slider .= '<label>Background Image</label>';
			$slider .= '<input type="text" class="slide of-input" name="'. $id .'['.$order.'][bg]" id="'. $id .'_'.$order .'_slide_bg" value="'. $val['bg'] .'" />';
			
			$slider .= '<div class="upload_button_div"><span class="button media_upload_button" id="'.$id.'_'.$order .'_bg" rel="' . $int . '">Upload</span>';
			
			if(!empty($val['bg'])) {$hide = '';} else { $hide = 'hide';}
			$slider .= '<span class="button mlu_remove_button '. $hide.'" id="reset_'. $id .'_'.$order .'_bg" title="' . $id . '_'.$order .'_bg">Remove</span>';
			$slider .='</div>' . "\n";
			$slider .= '<div class="screenshot">';
			if(!empty($val['bg'])){
				
		    	$slider .= '<a class="of-uploaded-image" href="'. $val['bg'] . '">';
		    	$slider .= '<img class="of-option-image" id="image_'.$id.'_'.$order .'_bg" src="'.$val['bg'].'" alt="" />';
		    	$slider .= '</a>';
		...
		$slider .= '<div class="slider_img">';
			$slider .= '<label>Slider Image</label>';
			$slider .= '<input type="text" class="slide of-input" name="'. $id .'['.$order.'][url]" id="'. $id .'_'.$order .'_slide_url" value="'. $val['url'] .'" />';
			
			$slider .= '<div class="upload_button_div"><span class="button media_upload_button" id="'.$id.'_'.$order .'_img" rel="' . $int . '">Upload</span>';
			
			if(!empty($val['url'])) {$hide = '';} else { $hide = 'hide';}
			$slider .= '<span class="button mlu_remove_button '. $hide.'" id="reset_'. $id .'_'.$order .'_img" title="' . $id . '_'.$order .'_img">Remove</span>';
			$slider .='</div>' . "\n";
			$slider .= '<div class="screenshot">';
			if(!empty($val['url'])){
				
				$slider .= '<a class="of-uploaded-image" href="'. $val['url'] . '">';
				$slider .= '<img class="of-option-image" id="image_'.$id.'_'.$order .'_img" src="'.$val['url'].'" alt="" />';
				$slider .= '</a>';
		...
		$slider .= '<label>Text Mode</label>';
		$slider .= '<div class="select_wrapper mini">';
		$slider .= '<select class="select of-input" name="'. $id .'['.$order.'][mode]" id="'. $id .'_'.$order .'_slide_mode">';
		foreach ($mode as $select_ID => $option) {			
			$slider .= '<option id="' . $select_ID . '" value="'.$select_ID.'" ' . selected($val['mode'], $select_ID, false) . ' />'.$option.'</option>';
		 } 
		$slider .= '</select></div>';
		
		$slider .= '<label>Description (optional)</label>';
		$slider .= '<textarea class="slide of-textarea" name="'. $id .'['.$order.'][description]" id="'. $id .'_'.$order .'_slide_description" cols="8" rows="8">'.stripslashes($val['description']).'</textarea>';
